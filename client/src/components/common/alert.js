...
const Alert = ({ alerts }) => (
    alerts.map((alert) => (
    <div id = "toast-success" className = "relative flex w-full max-w-xs p-4 mb-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800" role = "alert" style = {{ float: 'right' }} >
        <div className="flex items-center">
            ...
            {
                    <div key={ alert.id } className="ml-3 text-sm font-normal">
                        {alert.msg}
                    </div> 
            }
        </div>
